...
const (
	GuildID   = "1212233818669457449"
	AdminRole = "1212612300133896202"
)
...
func Run() {
	...
	discord.AddHandler(addReaction)
	discord.AddHandler(removeReaction)
	discord.AddHandler(editMessage)
...
func newMessage(discord *discordgo.Session, message *discordgo.MessageCreate) {
	...
	if isUserBlacklisted(message.Author.ID, "all") {
		return
	}

	isUserAdmin := func() bool {
		roles, err := discord.GuildMember(message.GuildID, message.Author.ID)
		if err != nil {
			return false
		}
	...
	case strings.Contains(message.Content, "!echo") && !strings.Contains(message.Content, " !echo"):

		_message := strings.Replace(message.Content, "!echo", "", 1)
		discord.ChannelMessageSend(message.ChannelID, _message)
		discord.ChannelMessageDelete(message.ChannelID, message.ID)

	case strings.Contains(message.Content, "!announce") && !strings.Contains(message.Content, " !announce"):

		if !isUserAdmin() {
			discord.ChannelMessageSend(message.ChannelID, "You are not allowed to use this command")
			return
		}

		_message := strings.Replace(message.Content, "!announce", "", 1)
		discord.ChannelMessageSend(message.ChannelID, "@everyone\n"+_message)
		discord.ChannelMessageDelete(message.ChannelID, message.ID)

	case strings.Contains(message.Content, "!rr") && !strings.Contains(message.Content, " !rr") && !strings.Contains(message.Content, "!rr_"):
		...
		content := strings.Replace(message.Content, "!rr", "", 1)
		_emojis := parseEmojis(content)
		_message, _ := discord.ChannelMessageSend(message.ChannelID, content)
		discord.ChannelMessageDelete(message.ChannelID, message.ID)

		for _, emoji := range _emojis {
			emoji = strings.Replace(emoji, "<", "", 1)
			emoji = strings.Replace(emoji, ">", "", 1)
			fmt.Println(emoji)
			discord.MessageReactionAdd(_message.ChannelID, _message.ID, emoji)
		}

	case strings.Contains(message.Content, "!rr_update") && !strings.Contains(message.Content, " !rr_update"):
		...
		content := strings.Replace(message.Content, "!rr_update", "", 1)
		discord.ChannelMessageEdit("1289650632915025981", "1289670103373316199", content)
		discord.ChannelMessageDelete(message.ChannelID, message.ID)
	}

}

func editMessage(discord *discordgo.Session, message *discordgo.MessageUpdate) {

	channel_id, message_id := message.ChannelID, message.ID
	reaction_message := channel_id == "1289650632915025981" && message_id == "1289670103373316199"

	if reaction_message {

		_emojis := parseEmojis(message.Content)

		for _, emoji := range _emojis {
			emoji = strings.Replace(emoji, "<", "", 1)
			emoji = strings.Replace(emoji, ">", "", 1)
			discord.MessageReactionAdd(channel_id, message_id, emoji)
		}
	}
}

func addReaction(discord *discordgo.Session, reaction *discordgo.MessageReactionAdd) {

	if isUserBlacklisted(reaction.UserID, "rr") || isUserBlacklisted(reaction.UserID, "all") {
		return
	}

	channel_id, message_id := reaction.ChannelID, reaction.MessageID
	reaction_message := channel_id == "1289650632915025981" && message_id == "1289670103373316199"

	emoji := reaction.Emoji.Name
	seniorRole, juniorRole, sophomoreRole, freshmanRole, juniorHighRole := "1212613988345446420", "1212614110739570809", "1212614169564676168", "1212614423017951283", "1212614494602133594"

	if reaction_message {
		switch emoji {
		case "1️⃣":
			discord.GuildMemberRoleAdd(GuildID, reaction.UserID, seniorRole)
		case "2️⃣":
			discord.GuildMemberRoleAdd(GuildID, reaction.UserID, juniorRole)
		case "3️⃣":
			discord.GuildMemberRoleAdd(GuildID, reaction.UserID, sophomoreRole)
		case "4️⃣":
			discord.GuildMemberRoleAdd(GuildID, reaction.UserID, freshmanRole)
		case "5️⃣":
			discord.GuildMemberRoleAdd(GuildID, reaction.UserID, juniorHighRole)
		}
	}
}

func removeReaction(discord *discordgo.Session, reaction *discordgo.MessageReactionRemove) {
...
func getBlacklist(uuid string) []string {
	...
	if blacklist, ok := db[uuid].([]interface{}); ok {
		var blacklistStr []string
		for _, v := range blacklist {
			blacklistStr = append(blacklistStr, v.(string))
		}
		return blacklistStr
	}
...
func isUserBlacklisted(uuid string, process string) bool {

	blacklist := getBlacklist(uuid)
	for _, v := range blacklist {
		if v == process {
			return true
		}
